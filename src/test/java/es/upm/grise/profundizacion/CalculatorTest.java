...
class CalculatorTest {
	...
	public void removeLastIndexOfValue () {
		...
		assertNotEquals(lastIndex, calculator.numbers.lastIndexOf(value), () -> "That existing index " + calculator.numbers.lastIndexOf(value)+ " is different from the original last index that has been removed");
	}
	...
	public void alreadyExistingMaxValue () {
		double max = 9;
		assertEquals(max, calculator.max(),  () -> "The value obtained is not the max from all the collection");
	}
	...
	public void alreadyExistingMinValue () {
		double min = 0;
		assertEquals(min, calculator.min(), () -> "The value obtained is not the min from all the collection");
	}
	...
	public void calculatingAverage () {
		double average = 4.4;
		assertEquals(calculator.average(), average, () -> "The average calculated by the method is not correct with the collection of numbers given. Expected: " + average + "; Result: " + calculator.average());
	}
	...
	public void calculatingStandardAverage () {
		...
		assertEquals(stddev, calculator.stddev(), () -> "The standard average calculated by the method is not correct with the collection of numbers given. Expected: " + stddev + "; Result: " + calculator.stddev());
	}
